...
/**
  ...
  */
  //1
  val pickupByTaxiZonesDF=taxiDF.groupBy("PULocationID")
    .agg(count("*").as("totalTrips"))
    .join(taxiZonesDF,col("PULocationID") === col("LocationID"))
    .drop("LocationID","service_zone")
    .orderBy(col("totalTrips").desc_nulls_last)
 // pickupByTaxiZonesDF.show()

  //1b -group by borough
  val pickupByBoroughDF=pickupByTaxiZonesDF.groupBy(col("Borough"))
...
//tripDistanceStatsDF.show()
  ...
  val tripByLengthDF=tripsWithLengthDF.groupBy("isLong").count()
  //tripByLengthDF.show()

  //4
  val pickupByHoursByLengthDF=tripsWithLengthDF
    .withColumn("hour_of_day",hour(col("tpep_pickup_datetime")))
    .groupBy("hour_of_day","isLong")
    .agg(count("*").as("totalTrips"))
    .orderBy(col("totalTrips").desc_nulls_last)

  pickupByHoursByLengthDF.show()
  //5.
